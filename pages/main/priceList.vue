<template>
    ...
    <view class="notice-text">你的价格最大波动率为<text class="red">{{totalcount}}</text>%</view>
    ...
    <view class="table">
			<z-table :tableData='PriceTableData' :columns='PriceColumns' stickSide='true'></z-table>
		</view>
    ...
    // 价格
		async getpricelist(today){
			...
			const result = await that.$api.interfaceApi('getpricelist')({
				...
				billtype: this.billtype, //1客户 2供应商
			});
			console.log(result)
			if (result.ret === 1) {
            ...
            if(that.billtype=='1'){ // 客户
              that.PriceColumns = that.PriceColumns
              that.PriceTableData = result.data.result

            }else if(that.billtype=='2'){ // 供应商
              that.PriceColumns = that.PriceSupplierColumns
              that.PriceTableData = result.data.result
