<template>
	<view :class="wrapClass">
		<image v-if="!needLogin" class="empty-content-image" :src="setSrc" mode="aspectFit"></image>
		<!-- 空白页 -->
		<template v-if="needLogin">
			...
			<auth ref="auth" :contentHeight="500" @hasLogin="hasLogined"></auth>
		</template>
	</view>
</template>
...
export default {
	...
	computed: {
		...
		wrapClass() {
			return this.needLogin ? 'empty' : 'empty-content';
		}
	},
	methods: {
		hasLogined(val) {
			console.log(val,'我是空空如也页面')
			this.$emit("runLogin",val)
		},
		navToLogin() {
			// #ifndef  MP-WEIXIN
			uni.navigateTo({
